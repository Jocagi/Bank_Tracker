"""add countries and movement location

Revision ID: c9d7e1f2a3b4
Revises: b8f6e2d3c5a1
...
def upgrade():
    ...
    op.create_index('ix_paises_codigo_iso', 'paises', ['codigo_iso'], unique=False)
    op.add_column('movimientos', sa.Column('pais_id', sa.Integer(), nullable=True))
    op.create_index('ix_movimientos_pais_id', 'movimientos', ['pais_id'], unique=False)
    with op.batch_alter_table('movimientos', schema=None) as batch_op:
        batch_op.create_foreign_key('fk_movimientos_pais_id', 'paises', ['pais_id'], ['id'])

    paises = sa.table(
        'paises',
        sa.column('nombre', sa.String(length=150)),
        sa.column('codigo_iso', sa.String(length=2)),
    )
    op.bulk_insert(paises, [
        {'nombre': nombre, 'codigo_iso': codigo_iso}
        for nombre, codigo_iso in ISO_COUNTRIES
    ])


def downgrade():
    with op.batch_alter_table('movimientos', schema=None) as batch_op:
        batch_op.drop_constraint('fk_movimientos_pais_id', type_='foreignkey')
        batch_op.drop_column('pais_id')
    op.drop_index('ix_movimientos_pais_id', table_name='movimientos')
    op.drop_index('ix_paises_codigo_iso', table_name='paises')
